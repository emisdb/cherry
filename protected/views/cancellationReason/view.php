<?php
/* @var $this CancellationReasonController */
/* @var $model CancellationReason */

$this->breadcrumbs=array(
	'Cancellation Reasons'=>array('index'),
	$model->name,
);

$this->menu=array(
	array('label'=>'List CancellationReason', 'url'=>array('index')),
	array('label'=>'Create CancellationReason', 'url'=>array('create')),
	array('label'=>'Update CancellationReason', 'url'=>array('update', 'id'=>$model->id)),
	array('label'=>'Delete CancellationReason', 'url'=>'#', 'linkOptions'=>array('submit'=>array('delete','id'=>$model->id),'confirm'=>'Are you sure you want to delete this item?')),
	array('label'=>'Manage CancellationReason', 'url'=>array('admin')),
);
?>

<h1>View CancellationReason #<?php echo $model->id; ?></h1>

<?php $this->widget('zii.widgets.CDetailView', array(
	'data'=>$model,
	'attributes'=>array(
		'id',
		'name',
		'value',
	),
)); ?>
